...
async function importComposable(config: any) {
  ...
  const { useVectorApiConfig } = await import('../../../src/presentation-v2/composables/useVectorApiConfig');
...
describe('useVectorApiConfig', () => {
  it('Embedding 缺失时阻止保存并返回错误', async () => {
    ...
    expect(vector.errors.value).toContain('缺少 embeddingEndpoint');
    expect(vector.errors.value).toContain('缺少 embeddingModel');
  ...
  it('Rerank endpoint/model 不成对时阻止保存', async () => {
    ...
    expect(vector.errors.value).toContain('rerankEndpoint 和 rerankModel 必须同时填写或同时留空');
  });
