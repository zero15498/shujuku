...
describe("AcuToastViewport", () => {
  it("renders stacked semantic toasts", async () => {
    ...
    const toasts = Array.from(el.querySelectorAll<HTMLElement>(".acu-toast"));
    ...
    expect(toasts[0].classList.contains("acu-toast--success")).toBe(true);
    expect(toasts[0].getAttribute("role")).toBe("status");
    expect(toasts[1].classList.contains("acu-toast--error")).toBe(true);
  ...
  it("dismiss button removes the toast item", async () => {
    ...
    const dismiss = el.querySelector<HTMLButtonElement>(".acu-toast__dismiss");
    expect(dismiss).not.toBeNull();
    dismiss!.click();
    await nextTick();

    expect(store.items).toHaveLength(0);
    expect(el.querySelector(".acu-toast")).toBeNull();
  });
  ...
  it("runs toast action and follows dismissOnClick", async () => {
    ...
    const actionButton = Array.from(el.querySelectorAll<HTMLButtonElement>(".acu-toast__action"))
