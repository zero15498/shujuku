...
import { settings_ACU, currentJsonTableData_ACU, getCurrentIsolationKey_ACU } from '../../service/runtime/state-manager';
...
import { getSummaryVectorIndexStats_ACU, inspectSummaryVectorIndexHealth_ACU } from '../../service/vector/summary-vector-index-storage-service';
import { clearSummaryVectorIndexTempCache_ACU, deleteCurrentSummaryVectorIndexForCurrentChat_ACU, tryRecoverSummaryVectorIndexFromExternalSnapshot_ACU } from '../../service/vector/summary-vector-index-cache-service';
...
export async function bindDataEvents_ACU(): Promise<void> {
      ...
      if ($vectorIndexDeleteCurrentButton_ACU.length) {
          $vectorIndexDeleteCurrentButton_ACU.off('click.acu_vector_index').on('click.acu_vector_index', async () => {
              ...
              const deleted = await deleteCurrentSummaryVectorIndexForCurrentChat_ACU();
              await refreshVectorIndexStatsPanel_ACU();
