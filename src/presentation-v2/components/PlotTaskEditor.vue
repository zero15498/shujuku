<template>
  <div v-if="task" class="acu-v2-plot-task-editor">
    <fieldset class="acu-v2-plot-task-editor__section">
      ...
      <div class="acu-v2-plot-task-editor__grid">
        <AcuFormRow label="任务名称">
          <AcuInput type="text" :model-value="task.name" placeholder="例如：记忆召回任务" @change="patch({ name: String($event) })" />
        </AcuFormRow>
        <AcuFormRow label="阶段号" hint="同阶段并发，跨阶段串行">
          <AcuInput type="number" :min="1" :step="1" :model-value="task.stage" @change="patch({ stage: Math.max(1, Math.round(Number($event))) })" />
        </AcuFormRow>
        <AcuFormRow label="最大重试">
          <AcuInput type="number" :min="1" :step="1" :model-value="task.maxRetries" @change="patch({ maxRetries: Math.max(1, Math.round(Number($event))) })" />
        </AcuFormRow>
      ...
      <div class="acu-v2-plot-task-editor__grid">
        <AcuFormRow label="标签摘取" hint="例如 recall,supplement，仅作用于本任务">
          <AcuInput type="text" :model-value="task.extractTags" @change="patch({ extractTags: String($event) })" />
        </AcuFormRow>
        <AcuFormRow label="提取注入标签" hint="优先级高于标签摘取；为空时不自动追加">
          <AcuInput type="text" :model-value="task.extractInjectTags" @change="patch({ extractInjectTags: String($event) })" />
        </AcuFormRow>
        <AcuFormRow label="最小回复长度" hint="少于此长度自动重试">
          <AcuInput type="number" :min="0" :step="10" :model-value="task.minLength" @change="patch({ minLength: Math.max(0, Math.round(Number($event))) })" />
        </AcuFormRow>
      </div>
    </fieldset>
    ...
    <fieldset class="acu-v2-plot-task-editor__section">
      ...
      <AcuFormRow
        label="API 预设"
        hint="通常保持继承；只有这个任务需要换模型、换额度或换接口时再单独选择。"
      >
        <AcuSelect
      ...
      <p class="acu-v2-plot-task-editor__hint">
        实际发送时会先看这里有没有单独选择；没有选择就使用剧情推进页里的 API 预设；如果剧情推进页也保持继承，就跟随当前活动 API。这个选择保存为全局偏好，不会写入预设 JSON。
      </p>
    </fieldset>
...
import type { PlotPromptSegment, PlotTaskDraft } from '../composables/usePlotTaskEditing';
...
.acu-v2-plot-task-editor__hint {
  margin: 0;
  font-size: 11px; color: var(--acu-text-3); line-height: 1.5;
}
