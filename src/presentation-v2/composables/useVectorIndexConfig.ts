 ...
 *   getCurrentVectorMemoryConfig_ACU / patchCurrentVectorMemoryConfig_ACU 操作。
...
export function useVectorIndexConfig() {
  ...
  >(key: K, raw: string | number): void {
    ...
    patchCurrentVectorMemoryConfig_ACU({ [key]: next });
    saveSettings_ACU();
    runValidation();
    pushSavedMessage();
  }
  ...
  >(key: K, raw: number | string): void {
    const min = key === 'recentFixedInjectCount' ? 0 : 1;
    const fallback = min === 0 ? 0 : 1;
    const num = Math.max(min, Math.floor(Number(raw)) || fallback);
    (form as any)[key] = num;
    patchCurrentVectorMemoryConfig_ACU({ [key]: num });
    saveSettings_ACU();
    runValidation();
    pushSavedMessage();
  }
  ...
  function setMinScore(raw: number | string): void {
    ...
    if (num > 1) num = 1;
    form.minScore = num;
    patchCurrentVectorMemoryConfig_ACU({ minScore: num });
    saveSettings_ACU();
    runValidation();
    pushSavedMessage();
  }
  ...
  function savePromptGroup(): void {
    ...
    patchCurrentVectorMemoryConfig_ACU({ keywordPromptGroup: segs });
    saveSettings_ACU();
    ...
    try {
      ...
      if (!snapshot?.summaryVectorIndexState?.manifest) {
        try {
          ...
          if (recovered) snapshot = getLatestSummaryVectorIndexSnapshotState_ACU();
        } catch {
          // 恢复失败只影响状态展示，不阻断页面。
        }
      }
